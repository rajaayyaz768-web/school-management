...
import { HardDrive, UserCog, Palette } from 'lucide-react'
import { cn } from '@/lib/utils'

const SETTINGS_NAV = [
  { label: 'Account',    href: '/principal/settings/account',    icon: UserCog },
  { label: 'Appearance', href: '/principal/settings/appearance', icon: Palette },
        ...
        {SETTINGS_NAV.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex items-center gap-2 px-[var(--space-4)] py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors',
                isActive
