...
export const fetchStaffForAttendance = async (
  ...
  const res = await axios.get('/api/v1/staff-attendance/staff-list', {
...
export const markDailyAttendance = async (
  ...
  const res = await axios.post('/api/v1/staff-attendance/mark', data)
  return res.data.data
}
...
export const fetchDailyReport = async (
  ...
  const res = await axios.get('/api/v1/staff-attendance/daily-report', {
    params: { campusId, date }
  })
  return res.data.data
}
...
export const fetchAbsentStaffToday = async (
  ...
  const res = await axios.get('/api/v1/staff-attendance/absent-today', {
    params: { campusId, date }
  })
  return res.data.data
}
